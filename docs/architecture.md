...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when `bin/fm-crew-state.sh` reports an exact busy verdict from the semantic busy-state contract.
...
For a planned ordinary ship task, that terminal reconciliation also invokes the non-executing handoff session-end review; [`handoff-admission.md`](handoff-admission.md) owns its evidence, SHA binding, refusal outcomes, and limits.
...
It consults semantic busy state: exact busy reports working, exact idle permits fallback to a status-log event whose verb maps to a recognized run-state, and unknown or a dead pane stays unknown instead of trusting a stale log.
...
## Handoff admission boundary

**Verified:** This fork does not run no-mistakes.
**Verified:** `bin/fm-handoff.sh` admits Git object content without checkout or host execution, and a session-end run review records static run evidence.
**Unverified:** The handoff gate is not a provenance system or a non-bypassable security control when a worker shares authority over the records it uses.
**Verified:** [handoff-admission.md](handoff-admission.md) owns the SHA-binding and content-closure contract.
