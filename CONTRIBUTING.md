...
Pull requests target `main` from an ordinary feature branch and remain subject to the repository's GitHub checks and explicit merge authority.
...
4. Run the focused behavior tests and lint for the changed surface.
5. Push your feature branch and open a pull request.

6. Wait for the configured merge authority before merging.
...
- Helper scripts in `bin/` are plain bash.
  ...
  `bin/fm-lint.sh` must pass: it is the single owner of the lint definition (the shellcheck file set, config, pinned shellcheck version, and pinned actionlint workflow lint), and CI runs its no-argument full-analysis path.
...
Crewmates route every `ask-user` finding to firstmate, which applies `ask-user-authority`.
The session-end handoff check is non-executing and does not replace focused behavior tests or CI.
Never hand-commit private fleet paths onto a feature branch; CI rejects them as tracked personal fleet paths.
