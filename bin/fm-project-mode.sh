...
case "$mode" in
  ...
  no-mistakes|no-mistakes-prod-only)
    echo "warn: retired mode \"$mode\" for $NAME; this fork does not run no-mistakes, so using direct-PR" >&2
    mode=direct-PR ;;
