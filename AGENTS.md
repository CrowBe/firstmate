...
state/               runtime records and signals; gitignored
  ...
  handoff/           plans, admission decisions, SHA-bound reviews, and secret-free run-review records; bin/fm-handoff.sh owns the contract, while quarantine copies live under state/handoff-quarantine/
...
Projects use `direct-PR` or `local-only` only.
...
This fork does not run no-mistakes.
The available paths are `direct-PR`, where the worker opens a PR, and `local-only`, where the worker stops with a clean ready branch for the guarded fast-forward path.
For every ordinary ship task, `bin/fm-handoff.sh` records a pre-launch base and output-ref plan.
It admits only a fully closed Git object graph without checking out or executing worker output, records a session-end run review, and requires an explicit review bound to the exact SHA before a local-only landing.
The handoff gate is a workflow guardrail, not a non-bypassable control when the worker shares authority over its state records; [`docs/handoff-admission.md`](docs/handoff-admission.md) owns the limits and evidence contract.
...
For a direct-PR ship task, the ready signal is `done: PR <url>` after the worker opens the PR.
