...
The handoff admission and session-end run-review boundary is summarized in [handoff-admission.md](handoff-admission.md), while `docs/sessionstart-nudge.md` covers the silent session-open hook use; each script header owns its exact contract.
...
| `fm-capsule.sh`          | See the private capsule operator contract in [containment.md](containment.md) |
...
| `fm-handoff.sh`          | Record and admit inert Git handoffs, then bind explicit review to a content SHA |
...
| `fm-gate-refuse-lib.sh`  | Shared retired-upstream gate-context refusal for fleet lifecycle entrypoints          |
...
| `fm-nm-run-lib.sh`       | Shared retired-upstream run-attribution compatibility library                        |
