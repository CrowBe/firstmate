...
retired mode value|--mode no-mistakes|This fork does not run no-mistakes
ROWS
...
# The retired delivery mode must fail before the scaffold writes any worker
# instructions that could direct a worker to invoke it.
test_retired_mode_is_refused() {
  local home id out status
  home="$TMP_ROOT/wording-home"
  mkdir -p "$home/data"
  id="brief-wording-b1"
  out=$(FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" some-proj --mode no-mistakes 2>&1); status=$?
  [ "$status" -ne 0 ] || fail "retired mode must not scaffold"
  assert_contains "$out" "This fork does not run no-mistakes" "retired mode refusal was not explicit"
  assert_absent "$home/data/$id/brief.md" "retired mode wrote a brief"
  pass "fm-brief.sh: retired mode is refused before it can direct a worker"
}
